...
def init_model(
    ...
    logger.info(f"Model {llm_config.model_id} is warming up...")
...
class LLMPredictor:
    ...
    async def rollover(self, scaling_config: ScalingConfig):
        ...
        async with self._new_worker_group_lock:
            ...
            self.new_worker_group = await self._create_worker_group(scaling_config)
    ...
    async def _create_worker_group(
        ...
        await asyncio.gather(
            *[
                ...
                for worker, local_rank in zip(worker_group, local_ranks, strict=True)
            ]
        )
